...
impl Database {
    ...
    fn migrate_v15_to_v16(conn: &Connection) -> Result<(), AppError> {
        ...
        legacy_reset_codex_usage_on_conn(conn, &codex_dir)
    }
...
// SQL migration compatibility for old session-derived rows; no files are read.
fn legacy_is_rollout_filename(file_name: &str) -> bool {
...
fn legacy_is_codex_cursor_path(file_path: &str, codex_dir: &Path) -> bool {
    let path = Path::new(file_path);
    let file_name = file_path.rsplit(['/', '\\']).next().unwrap_or_default();
    if !legacy_is_rollout_filename(file_name) {
        return false;
    }
...
fn legacy_sqlite_table_exists(conn: &rusqlite::Connection, table: &str) -> Result<bool, AppError> {
    conn.query_row(
...
fn legacy_sqlite_column_exists(
...
pub(crate) fn legacy_reset_codex_usage_on_conn(
    conn: &rusqlite::Connection,
    codex_dir: &Path,
) -> Result<(), AppError> {
    if legacy_sqlite_table_exists(conn, "proxy_request_logs")?
        && legacy_sqlite_column_exists(conn, "proxy_request_logs", "data_source")?
    {
        conn.execute(
    ...
    if legacy_sqlite_table_exists(conn, "usage_daily_rollups")?
        && legacy_sqlite_column_exists(conn, "usage_daily_rollups", "provider_id")?
    {
        conn.execute(
    ...
    if legacy_sqlite_table_exists(conn, "session_log_sync")?
        && legacy_sqlite_column_exists(conn, "session_log_sync", "file_path")?
    {
        let paths = {
        ...
        for file_path in paths
            .into_iter()
            .filter(|path| legacy_is_codex_cursor_path(path, codex_dir))
        {
            conn.execute(
