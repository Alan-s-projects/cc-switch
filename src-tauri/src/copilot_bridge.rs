...
fn setup_suggestion_from_file(
    ...
    setup_suggestion(&text, path, exists, endpoint, catalog).map_err(|error| error.to_string())
}
...
mod tests {
    ...
    fn selected_files_have_independent_snapshots_without_changing_other_files() {
        ...
        for ((name, text), effort, quote) in [
            ...
            let preview =
                setup_suggestion_from_file(&selected, true, "http://127.0.0.1:15721/v1", None)
                    .unwrap();
    ...
    fn missing_auto_file_is_distinct_from_missing_selection_and_existing_empty_file() {
        ...
        let preview =
            setup_suggestion_from_file(&path, false, "http://127.0.0.1:15721/v1", None).unwrap();
        assert!(!preview.config_exists);
        ...
        assert!(setup_suggestion_from_file(&path, true, "http://127.0.0.1:15721/v1", None)
            .unwrap_err()
        ...
        for explicit in [true, false] {
            let preview =
                setup_suggestion_from_file(&empty, explicit, "http://127.0.0.1:15721/v1", None)
                    .unwrap();
            assert!(preview.config_exists);
            assert_diff_snapshots(&preview.copilot_lines, "", &preview.copilot_config);
    ...
    fn invalid_preview_paths_and_files_fail_without_rewriting_them() {
        ...
        assert!(setup_suggestion_from_file(&folder, true, "http://localhost/v1", None).is_err());

        let broken = directory.path().join("broken.toml");
        std::fs::write(&broken, "broken = [").unwrap();
        assert!(setup_suggestion_from_file(&broken, true, "http://localhost/v1", None)
            .unwrap_err()
            .contains("invalid"));
        ...
        assert!(setup_suggestion_from_file(&unreadable, true, "http://localhost/v1", None)
            .unwrap_err()
...
"#;
        ...
        let preview =
            setup_suggestion_from_file(&path, true, "http://127.0.0.1:15721/v1", Some(&catalog))
                .unwrap();
        assert!(preview.config_exists);
    ...
    fn handles_inline_tables_missing_providers_and_reserved_ids() {
        for text in [
            ...
            let preview = setup_suggestion(text, Path::new("config.toml"), false, "http://127.0.0.1:15721/v1", None).unwrap();
...
"#;
        ...
        std::fs::write(&path, text).unwrap();
        let result =
            setup_suggestion_from_file(&path, true, "http://127.0.0.1:15721/v1", None).unwrap();
    ...
    fn suggestion_handles_fresh_and_official_configs_without_creating_a_file() {
        ...
        for text in ["", "model_provider = \"openai\"\n"] {
            let result = setup_suggestion(text, &path, false, "http://127.0.0.1:15721/v1", None).unwrap();
        ...
        assert!(setup_suggestion("broken = [", &path, false, "http://localhost/v1", None).is_err());
        assert!(!path.exists());
        let official = "model_provider = \"openai\"\n[model_providers.copilot]\nbase_url = \"http://127.0.0.1:15721/v1\"\n";
        assert!(
            !setup_suggestion(official, &path, false, "http://127.0.0.1:15721/v1", None)
                .unwrap()
                .configured
        );
    }
