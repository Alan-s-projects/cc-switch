...
export function flattenModels(data: ModelsDevResponse): ModelsDevEntry[] {
  ...
  for (const [providerId, provider] of Object.entries(data)) {
    if (providerId !== "openai" || !provider || typeof provider !== "object")
      continue;
    const providerName = provider.name || providerId;
    for (const [modelId, model] of Object.entries(provider.models ?? {})) {
      if (!isGptModel(modelId) || !isTextPricingModel(modelId, model)) continue;
...
const COMMON_MODEL_LIMIT = 6;

/** Select a bounded set of recent canonical GPT coding/chat prices. */
export function getCommonModelKeys(entries: ModelsDevEntry[]): Set<string> {
  return new Set(
    entries
      .filter(
        (entry) => entry.providerId === "openai" && isGptModel(entry.modelId),
      )
